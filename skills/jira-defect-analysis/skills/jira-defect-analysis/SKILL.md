...
| 字段 | 判断逻辑 |
|------|---------|
| 発生原因 | 代码写错→不具合 / 客户需求变化→仕様変更 / 设计阶段遗漏→設計漏れ |
| 解決方法 | 改 bug→修正 / 新功能→追加開発 / 改配置→設定変更 / 确认不是问题→対応不要 |
| 処置区分 | 改逻辑/功能→BUG修正 / 改需求规格→仕様変更 / 只改文案→文言修正 |
| 不具合区分 | 接口参数/返回值错误→IF誤り / 代码逻辑错误→記述誤り / 规格描述不完整→仕様不備 |
| 作り込み工程 | 需求分析阶段引入→要件定義 / 设计阶段引入→設計 / 编码阶段引入→実装 |
| 発見すべき工程 | 设计评审能发现→設計レビュー / 单元测试能发现→単体テスト / 集成测试能发现→結合テスト |

##### 詳細フィールド（自由文）の書き方ルール

> **面向非技术日语母语用户，简洁说明，复杂问题不超过30字。**
> 禁止写：API名、URL、函数名、英文技术术语。
> 正确示例：「Jiraのシステム仕様変更により、検索機能が動作しなくなった」
> 错误示例：「/rest/api/3/searchエンドポイントが410エラーを返した」
